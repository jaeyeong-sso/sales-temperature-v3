...
public class SalesVolumeDao {
	...
	public List<SalesVolume> listingMonthlySalesVolume(){
		
		String queryStr = "SELECT tr_date, SUM(num_of_product) AS num_of_product, SUM(sales_amount) AS total_amount " +
		        "FROM ( SELECT SUBSTR(date_receipt_num,1,7) AS tr_date, num_of_product, sales_amount FROM ext_tr_receipt WHERE SUBSTR(date_receipt_num,1,4) = '2014') view_tr_recipt " +
				"GROUP BY tr_date ORDER BY tr_date ASC";
		
		return this.jdbcTemplate.query(queryStr, this.salesVolumeListMapper);
	}
	
	public Map<String, Object> getAnnualSalesVolume(){
        ...
        "FROM (" +
            ...
            "FROM ext_tr_receipt WHERE SUBSTR(date_receipt_num,1,4) = '2014') view_tr_receipt " +
